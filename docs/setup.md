...
| The log shows `exec … stream dropped by peer` although the network is fine | A local client closed its connection before the answer ended (Ctrl-C, a client timeout, a closed tab). Local requests also reach `/exec` over loopback, so this is logged the same way as a vanished peer |
