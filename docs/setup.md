...
| A client that stops reading for a while is disconnected | On Linux the daemon drops a connection whose sent data stays unacknowledged for 25 s (macOS: ~25–45 s); this also frees the model slot when a peer vanishes. Read the stream continuously |
