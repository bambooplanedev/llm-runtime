...
Використання: make_gguf.py out.gguf --arch qwen3 --layers 28 --kv-heads 8 --head-dim 128
             [--params-per-layer N] [--experts 128 --experts-used 8] [--split 1/3]
...
if a.split:
    ...
    kvs += [kv_u16("split.no", no - 1), kv_u16("split.count", cnt)]
...
hdr = b"GGUF" + struct.pack("<I", 3) + struct.pack("<Q", a.layers) + struct.pack("<Q", len(kvs))
